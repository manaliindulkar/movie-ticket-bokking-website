...
def Home(request):
    data = Movie.objects.order_by('-release_date')
    d = {'data':data}
...
def All_Movies(request,pid):
    ...
    if pid == 1:
        ...
        data = Movie.objects.filter(language=lang).order_by('-release_date')
    if pid == 2:
    ...
    if pid == 3:
        ...
        data = Movie.objects.filter(language=lang).order_by('-release_date')
    if pid == 4:
        language = "Marathi Movies"
        lang = Language.objects.get(name="Marathi")
        data = Movie.objects.filter(language=lang).order_by('-release_date')
